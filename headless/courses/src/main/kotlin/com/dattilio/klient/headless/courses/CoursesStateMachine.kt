...
class CoursesStateMachine constructor(
    ...
    sealed class SideEffect(val timeoutEvent: Event.TimeoutEvent? = null) {
        object Stand : SideEffect(
            Event.TimeoutEvent(
                Timeout.StandTimeout
            )
        )
        object East : SideEffect(
            Event.TimeoutEvent(
                Timeout.EastTimeout
            )
        )
        object ClimbRope : SideEffect(
            Event.TimeoutEvent(
                Timeout.ClimbTimeout
            )
        )
        object GoPlank : SideEffect(
            Event.TimeoutEvent(
                Timeout.PlankTimeout
            )
        )
        object GoPath : SideEffect(
            Event.TimeoutEvent(
                Timeout.PathTimeout
            )
        )
        ...
        object South : SideEffect(
            Event.TimeoutEvent(
                Timeout.SouthTimeout
            )
        )
        object Rope4 : SideEffect(
            Event.TimeoutEvent(
                Timeout.Rope4Timeout
            )
        )
        object Path4 : SideEffect(
            Event.TimeoutEvent(
                Timeout.Path4Timeout
            )
        )
        object Track4 : SideEffect(
            Event.TimeoutEvent(
                Timeout.Track4Timeout
            )
        )
        object Coal4 : SideEffect(
            Event.TimeoutEvent(
                Timeout.Coal4Timeout
            )
        )
    ...
    val stateMachine = StateMachine.create<State, Event, SideEffect> {
        ...
        state<State.Idle> {
            on<Event.ArriveRope> { transitionTo(
                State.Rope,
                SideEffect.ClimbRope
            ) }
            on<Event.ArrivePlank> { transitionTo(
                State.Plank,
                SideEffect.GoPlank
            ) }
            on<Event.ArrivePath> { transitionTo(
                State.Path,
                SideEffect.GoPath
            ) }

            on<Event.ArriveRope4> { transitionTo(
                State.Rope4,
                SideEffect.Rope4
            ) }
            on<Event.ArrivePath4> { transitionTo(
                State.Path4,
                SideEffect.Path4
            ) }
            on<Event.ArriveTrack4> { transitionTo(
                State.Track4,
                SideEffect.Track4
            ) }
            on<Event.ArriveCoal4> { transitionTo(
                State.Coal4,
                SideEffect.Coal4
            ) }
        }
        state<State.Start> {
            ...
            on<Event.StandSuccess> { transitionTo(
                State.Start,
                SideEffect.CancelTimeout(
                    Timeout.StandTimeout
                )
            ) }
            on<Event.Success> { transitionTo(
                State.Rope,
                SideEffect.East
            ) }
            on<Event.Failed> { transitionTo(
                State.Start,
                SideEffect.Stand
            ) }
        ...
        state<State.Rope> {
            on<Event.NoLongerBusy> { transitionTo(
                State.Rope,
                SideEffect.ClimbRope
            ) }
            on<Event.ArriveRope> { transitionTo(
            ...
            on<Event.StartedClimbing> { transitionTo(
                State.Rope,
                SideEffect.CancelTimeout(
                    Timeout.ClimbTimeout
                )
        ...
        state<State.Plank> {
            ...
            on<Event.StartedPlank> { transitionTo(
                State.Plank,
                SideEffect.CancelTimeout(
                    Timeout.PlankTimeout
                )
        ...
        state<State.Path> {
            ...
            on<Event.StartedPath> { transitionTo(
                State.Path,
                SideEffect.CancelTimeout(
                    Timeout.PathTimeout
                )
        ...
        state<State.Rope4> {
            ...
            on<Event.StartedRope4> { transitionTo(
                State.Rope4,
                SideEffect.CancelTimeout(
                    Timeout.Rope4Timeout
                )
        ...
        state<State.Path4> {
            ...
            on<Event.StartedPath4> { transitionTo(
                State.Path4,
                SideEffect.CancelTimeout(
                    Timeout.Path4Timeout
                )
        ...
        state<State.Track4> {
            ...
            on<Event.StartedTrack4> { transitionTo(
                State.Track4,
                SideEffect.CancelTimeout(
                    Timeout.Track4Timeout
                )
            ...
            on<Event.Failed> { transitionTo(
                State.Start,
                SideEffect.Failed.Track4Failed(
                    ++track4FailureCount
                )
            ) }
            on<Event.TimeoutEvent> {
        ...
        state<State.Coal4> {
            ...
            on<Event.StartedCoal4> { transitionTo(
                State.Coal4,
                SideEffect.CancelTimeout(
                    Timeout.Coal4Timeout
                )
